...
#include "Pred.h"
#include "Tools/Hacks/Backtrack.h"
#include "Tools/Hacks/Resolver.h"
...
void ResolverPlayerHurtEvent(IGameEvent* event)
{
	if (Options.Ragebot.HvH.bResolver)
	{
		if (Interfaces.pEngine->GetPlayerForUserID(event->GetInt("attacker")) == Interfaces.pEngine->GetLocalPlayer())
		{
			...
			pResolverData[i].missedshots = 0;
			pResolverData[i].playerhurtcalled = true;
			//pResolverData[i].addyaw += 45;
			/*if (pResolverData[i].addyaw >= 120)
				pResolverData[i].addyaw = 0;
...
void ResolverWeaponFireEvent(IGameEvent* event)
{
	if (Options.Ragebot.HvH.bResolver)
	{
		if (Interfaces.pEngine->GetPlayerForUserID(event->GetInt("userid")) == Interfaces.pEngine->GetLocalPlayer())
		{
			CBaseEntity* pEntity = Interfaces.pEntList->GetClientEntity(Interfaces.pEngine->GetPlayerForUserID(event->GetInt("userid")));
			int i = pEntity->GetIndex();
			pResolverData[i].weaponfirecalled = true;
		}
	}
}

void ResolverPlayerDeathEvent(IGameEvent* event)
{
	if (Options.Ragebot.HvH.bResolver)
	{
		if (Interfaces.pEngine->GetPlayerForUserID(event->GetInt("attacker")) == Interfaces.pEngine->GetLocalPlayer())
...
CGameEventListener* ResolverPlayerDeath;
CGameEventListener* ResolverWeaponFire;

void Init()
{
	REG_EVENT_LISTENER(Hurts, &Hurt, "player_hurt", false);
	REG_EVENT_LISTENER(ResolverPlayerHurt, &ResolverPlayerHurtEvent, "player_hurt", false);
	REG_EVENT_LISTENER(HitSounds, &HitSound, "player_death", false);
	REG_EVENT_LISTENER(ResolverWeaponFire, &ResolverWeaponFireEvent, "weapon_fire", false);
	//REG_EVENT_LISTENER(ResolverPlayerDeath, &ResolverPlayerDeathEvent, "player_death", false);
}
...
bool __fastcall HookedIsPlayingTimeDemo(void* thisptr, void* edx) {
	bool bResult = Hacks.g_fnOriginalPlayingTimeDemo(thisptr);
...
{
	AA.ShouldAA = false;
	CreateMoveETC::GetCmds(sequence_number, Hacks.CurrentCmd, Hacks.VerifiedCmd);
	Aimbot.Target = Vector(0, 0, 0);

	Misc::ServerRankReveal();

	if (Hacks.CurrentCmd && Hacks.VerifiedCmd && Interfaces.pEngine->IsConnected() && Interfaces.pEngine->IsInGame() && Hacks.LocalPlayer)
	{
		...
		if (Hacks.LocalWeapon)
		{
			if (Hacks.LocalPlayer->isAlive())
			{
				...
				if (enable)
				{
					if (Options.Legitbot.Aimbot.Heavy.bEnableLegitBacktrack || Options.Legitbot.Aimbot.Shotguns.bEnableLegitBacktrack || Options.Legitbot.Aimbot.Smgs.bEnableLegitBacktrack || Options.Legitbot.Aimbot.Rifles.bEnableLegitBacktrack || Options.Legitbot.Aimbot.Snipers.bEnableLegitBacktrack || Options.Legitbot.Aimbot.Pistols.bEnableLegitBacktrack)
						backtracking->legitBackTrack(Hacks.CurrentCmd, Hacks.LocalPlayer);
				}
				//CreateMoveETC::LocalPrediction();
				CreateMoveETC::EnginePrediction(Hacks.CurrentCmd);
				LegitBot.Run();
				Trigger.Run();
		...
		CreateMoveETC::VerifyCmd(Hacks.VerifiedCmd);
		//plist.UpdateSim();
	}
}
