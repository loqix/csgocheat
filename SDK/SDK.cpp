...
bool TraceToExit2(Vector& vecEnd, trace_t* pEnterTrace, Vector vecStart, Vector vecDir, trace_t* pExitTrace)
{
	...
	{
		return m_pEnt != nullptr && m_pEnt == Interfaces.pEntList->GetClientEntity(0);
	};
	...
	while (distance <= 90.0f) {
		...
		auto point_contents = Interfaces.pTrace->GetPointContents(vecEnd, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr);
		...
		Interfaces.pTrace->TraceRay(ray, 0x4600400B, nullptr, pExitTrace);
		...
		if (pExitTrace->startsolid && pExitTrace->surface.flags & SURF_HITBOX) {
			...
			Interfaces.pTrace->TraceRay(ray, 0x600400B, &filter, pExitTrace);
		...
		if (!(pExitTrace->fraction < 1.0 || pExitTrace->allsolid || pExitTrace->startsolid) || pExitTrace->startsolid) {
			if (pExitTrace->m_pEnt) {
				...
				if (dwEntBreakable == NULL) {
					dwEntBreakable = (DWORD)Utils.PatternScan(GetModuleHandleA("client.dll"), "55 8B EC 51 56 8B F1 85 F6 74 68");
				}
				bool bBreakable;
...
bool TraceToExit(Vector& end, trace_t& tr, Vector start, Vector vEnd, trace_t* trace)
{
	...
	while (distance <= 90.0f) {
		distance += 4.0f;
		end = start + vEnd * distance;

		auto point_contents = Interfaces.pTrace->GetPointContents(vEnd, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr);
		if (point_contents & MASK_SHOT_HULL && (!(point_contents & CONTENTS_HITBOX)))
			continue;
		...
		Interfaces.pTrace->TraceRay(ray, 0x4600400B, nullptr, trace);

		if (trace->m_pEnt == nullptr)
			return false;

		if (trace->startsolid && trace->surface.flags & SURF_HITBOX) {
			CTraceFilter filter;
			filter.pSkip = trace->m_pEnt;

			ray.Init(end, start);
			Interfaces.pTrace->TraceRay(ray, 0x600400B, &filter, trace);
		...
		if (!(trace->fraction < 1.0 || trace->allsolid || trace->startsolid) || trace->startsolid) {
			if (trace->m_pEnt) {
				...
				if (!DidHitNonWorldCBaseEntity(trace->m_pEnt) && /*GameUtils::IsBreakableEntity(pExitTrace->m_pEnt) &&*/ bBreakable)
					return true;
			}
		...
		if (((trace->surface.flags >> 7) & 1) && !((trace->surface.flags >> 7) & 1))
			continue;

		if (trace->plane.normal.Dot(vEnd) < 1.1f) {
			float fraction = trace->fraction * 4.0f;
