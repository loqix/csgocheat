...
public:
	...
	static bool FireSimulatedBullet( FireBulletData& data, CBaseEntity* Local, CBaseCombatWeapon* weapon )
	{
		...
		CCSWeaponInfo* Weapon_Info = weapon->GetCSWpnData();
		...
		while( ( data.penetrate_count > 0 ) && ( data.current_damage >= 1.0f ) )
		{
			...
			if( ( data.enter_trace.hitgroup <= 7 ) && ( data.enter_trace.hitgroup > 0 ) )
			{
				...
				ScaleDamage(data.enter_trace.hitgroup, data.enter_trace.m_pEnt, Weapon_Info->flArmorRatio, data.current_damage);
				return true;
			}
