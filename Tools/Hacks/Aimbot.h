		...
		if (iArray.size() > 0)
		{
			for (int i = 0; i < iArray.size(); i++)
			{
				...
				if (!box.GetHitbox(pEntity, iArray[i]))
					continue;
				//RageBackData[pEntity->GetIndex()][Hacks.CurrentCmd->command_number % 15].damage = box.GetBestPoint(RageBackData[pEntity->GetIndex()][Hacks.CurrentCmd->command_number % 15].hitboxPos);
