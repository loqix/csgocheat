...
private:
	...
	void FakeWalk()
	{
		if (KEY_DOWN(Options.Ragebot.AntiAim.Misc.iFakeWalkKey) && Options.Ragebot.AntiAim.Misc.bFakeWalk)
		{
			...
			{
				float velocity2 = Hacks.LocalPlayer->GetVecVelocity().Length2D();
				if (Hacks.CurrentCmd->command_number % 16 < 16)
				{
					if (velocity2 <= 0)
					{
						...
						Hacks.CurrentCmd->forwardmove = 449;
					}
					else if (velocity2 >= 10) {
						...
						QAngle direction{};
						g_Math.VectorAngles3D(velocity, direction);
					...
					{
						Hacks.SendPacket = false;
						Hacks.CurrentCmd->forwardmove = Hacks.CurrentCmd->sidemove = 0;
					}
				}
				/*else if (velocity2 <= 0)
				{
