...
void DrawLegitTab()
{
	...
	if (Global::LegitSubTab == 0)
	{
		ImGui::BeginChild("Pistols##aimbot", ImVec2(200, 398), true);
		{
			...
			ImGui::Checkbox("Enable Legit Backtrack", &Options.Legitbot.Aimbot.Pistols.bEnableLegitBacktrack);
	...
	if (Global::LegitSubTab == 1)
	{
		ImGui::BeginChild("Smgs##aimbot", ImVec2(200, 398), true);
		{
			...
			ImGui::Checkbox("Enable Legit Backtrack", &Options.Legitbot.Aimbot.Smgs.bEnableLegitBacktrack);
	...
	if (Global::LegitSubTab == 2)
	{
		ImGui::BeginChild("Rifles##aimbot", ImVec2(200, 398), true);
		{
			...
			ImGui::Checkbox("Enable Legit Backtrack", &Options.Legitbot.Aimbot.Rifles.bEnableLegitBacktrack);
	...
	if (Global::LegitSubTab == 3)
	{
		ImGui::BeginChild("Shotguns##aimbot", ImVec2(200, 398), true);
		{
			...
			ImGui::Checkbox("Enable Legit Backtrack", &Options.Legitbot.Aimbot.Shotguns.bEnableLegitBacktrack);
	...
	if (Global::LegitSubTab == 4)
	{
		ImGui::BeginChild("Heavy##aimbot", ImVec2(200, 398), true);
		{
			...
			ImGui::Checkbox("Enable Legit Backtrack", &Options.Legitbot.Aimbot.Heavy.bEnableLegitBacktrack);
	...
	if (Global::LegitSubTab == 5)
	{
		ImGui::BeginChild("Snipers##aimbot", ImVec2(200, 398), true);
		{
			...
			ImGui::Checkbox("Enable Legit Backtrack", &Options.Legitbot.Aimbot.Snipers.bEnableLegitBacktrack);
...
void DrawVisualsTab()
{
	...
	ImGui::BeginChild("Visuals - Misc", ImVec2(190, 398), true);
	{
		...
		ImGui::Separator();
		ImGui::Checkbox("BackTrack Dot", &Options.Visuals.VisualsMisc.bBackTrackDot);
		ImGui::NewLine();
		ImGui::SameLine(28);
		...
		ImGui::Begin("Infobox", &Options.Visuals.Infobox.bInfoBox, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_ShowBorders);
		{
			if (Interfaces.pEngine->IsInGame() && Interfaces.pEngine->IsConnected() && Hacks.LocalPlayer->isAlive())
			{
				...
				if (Options.Visuals.Infobox.bInfoLby)
				{
					...
					static float simtime{}, simtimechoke{};
					if (Hacks.SendPacket)
						simtime = Hacks.LocalPlayer->GetSimulationTime();
					else
						simtimechoke = Hacks.LocalPlayer->GetSimulationTime();
					ImGui::Text("Sim Time: %f", simtime);
					ImGui::Text("Sim Time Choke: %f", simtimechoke);
					ImGui::Text("Old Sim Time: %f", Hacks.LocalPlayer->GetOldSimulationTime());
					ImGui::Text("Old Sim Time Average: %f", fabs(Hacks.LocalPlayer->GetOldSimulationTime() - simtime));
#define TICK_INTERVAL			(Interfaces.pGlobalVars->interval_per_tick)
#define TIME_TO_TICKS2( dt )		( (int)( 0.5f + (float)(dt) / TICK_INTERVAL ) )
					static int ticks[6];
					ImGui::Text("Old Sim Tick Average: %i", TIME_TO_TICKS2(fabs(Hacks.LocalPlayer->GetOldSimulationTime() - Hacks.LocalPlayer->GetSimulationTime())));
					ImGui::Text("%i", TIME_TO_TICKS2(fabs(Hacks.LocalPlayer->GetOldSimulationTime() - Hacks.LocalPlayer->GetSimulationTime())));
					ticks[Hacks.CurrentCmd->command_number % 6] = TIME_TO_TICKS2(fabs(Hacks.LocalPlayer->GetOldSimulationTime() - Hacks.LocalPlayer->GetSimulationTime()));
					for (int i = 1; i < 7; i++)
					{
						ImGui::Text("[%i]Tick SimTime: %i", i, ticks[i]);
					}
					//ImGui::Text("Using Fake Angles: %i", );
					/*for (int w = 0; w < 15; w++)
					{
						AnimationLayer currentLayer = Hacks.LocalPlayer->GetAnimOverlay(w);
						const int activity = Hacks.LocalPlayer->GetSequenceActivity(currentLayer.m_nSequence);
						...
						ImGui::Text("sequence: %lu", currentLayer.m_nSequence);
						ImGui::SameLine();
						ImGui::Text("norder: %i", norder);
						ImGui::SameLine();
						ImGui::Text("FlCycle: %f", flcycle);
						ImGui::SameLine();
						ImGui::Text("flprevcycle: %f", flprevcycle);
						ImGui::SameLine();
						ImGui::Text("flweight: %f", flweight);
						ImGui::SameLine();
						ImGui::Text("flweightdatarate: %f", flweightdatarate);
						ImGui::SameLine();
						ImGui::Text("activity: %i", activity);
					}*/
				}
				...
				if (Options.Visuals.Infobox.bInfoAngles)
				{
					...
					ImGui::Text("Tick_Count: %i", Hacks.CurrentCmd->tick_count);
